	...
	if (opt.classifier == 'rnn') then
		...
		if opt.bidirectional then
			if (opt.additional_features and opt.memm_layer) then
				model, lstm_model, crit, sequencers = bidirectionalRNNmodelExtraFeaturesMEMM(num_features, opt.embedding_size, nclasses, opt.rnn_unit1, opt.rnn_unit2, opt.dropout, opt.cuda, opt.hidden, opt.bidirectional_layers, nclasses, opt.sequence_length, opt.b)
